...
console.log(
    chalk.bold.blue('Xzlyne') + chalk.bold.green('bailyes')
);
...
async function validateMembership(key) {
    ...
    try {
        // Meneruskan pengecekan ke web utama via Netlify Forwarder
...
/**
 * Wrapper makeWASocket dengan Filter Keamanan
 */
exports.makeWASocket = async (config) => {
    // Ambil authKey dari config atau env
    const authKey = config?.authKey || global.authKeymd || process.env.XZLYNN_AUTH;
    ...
    if (check.status === true) {
        ...
        throw new Error("XZLYNEBAILYES_ACCESS_DENIED");
    }
};
