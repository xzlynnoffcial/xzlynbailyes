...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child === null || child === void 0 ? void 0 : child.tag) {
            ...
            case 'revoked_membership_requests':
                ...
                break;
            default:
            // console.log("XZLYNEBAILYES-DEBUG:", JSON.stringify({ ...child, content: Buffer.isBuffer(child.content) ? child.content.toString() : child.content, participant }, null, 2))
        }
    };
    ...
    const handleReceipt = async (node) => {
        ...
        // Register LID→PN mapping dari receipt
        if (attrs.from?.endsWith('@lid') && attrs.sender_pn) {
            const { registerLidJid } = require('../WABinary/jid-utils');
            registerLidJid(attrs.from, attrs.sender_pn);
        }
        if (attrs.sender_lid && attrs.from && !attrs.from.endsWith('@lid')) {
            const { registerLidJid } = require('../WABinary/jid-utils');
            registerLidJid(attrs.sender_lid, attrs.from);
        }
    ...
    const handleNotification = async (node) => {
        ...
        // Register LID→PN mapping sebelum cek shouldIgnoreJid
        if (remoteJid?.endsWith('@lid') && node.attrs.sender_pn) {
            const { registerLidJid } = require('../WABinary/jid-utils');
            registerLidJid(remoteJid, node.attrs.sender_pn);
        }
        if (shouldIgnoreJid(remoteJid) && remoteJid !== '@s.whatsapp.net') {
    ...
    const handleMessage = async (node) => {
        var _a, _b, _c;
        // Register LID→PN mapping sebelum cek shouldIgnoreJid
        if (node.attrs.from?.endsWith('@lid') && node.attrs.sender_pn) {
            const { registerLidJid } = require('../WABinary/jid-utils');
            registerLidJid(node.attrs.from, node.attrs.sender_pn);
        }
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        let type = undefined;
                        // ── LID→PN: konversi participant ──
                        if ((_b = msg.key.participant) === null || _b === void 0 ? void 0 : _b.endsWith('@lid')) {
                            msg.key.participant = msg.key.participantAlt || node.attrs.participant_pn || msg.key.senderPn || authState.creds.me.id;
                        }
                        // ── LID→PN: konversi remoteJid non-group ──
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            msg.key.remoteJid = msg.key.remoteJidAlt || node.attrs.sender_pn || node.attrs.peer_recipient_pn || msg.key.remoteJid;
                        }
                        // ── LID→PN: konversi contextInfo.participant di SEMUA tipe pesan ──
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && msg.message) {
                            const msgType = Object.keys(msg.message).filter(k => k !== 'messageContextInfo' && k !== 'senderKeyDistributionMessage')[0];
                            const innerContent = msgType && msg.message[msgType];
                            if (innerContent && typeof innerContent === 'object' && innerContent.contextInfo) {
                                const ctxParticipant = innerContent.contextInfo.participant;
                                if (ctxParticipant && ctxParticipant.endsWith('@lid')) {
                                    try {
                                        const metadata = await groupMetadata(msg.key.remoteJid);
                                        const found = metadata.participants.find(p => p.id === ctxParticipant || p.lid === ctxParticipant);
                                        innerContent.contextInfo.participant = (found === null || found === void 0 ? void 0 : found.id) || ctxParticipant.replace('@lid', '@s.whatsapp.net');
                                    } catch (e) {
                                        innerContent.contextInfo.participant = ctxParticipant.replace('@lid', '@s.whatsapp.net');
                                    }
                                }
                            }
                        }
