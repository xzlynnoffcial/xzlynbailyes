...
const makeSocket = (config) => {
    ...
    // ⚡ Changed from const to let — needed for auto-reconnect
    let ws = new Client_1.WebSocketClient(url, config);
    ws.connect();
    const ev = (0, Utils_1.makeEventBuffer)(logger);
    /** ephemeral key pair used to encrypt/decrypt communication. Unique for each connection */
    let ephemeralKeyPair = Utils_1.Curve.generateKeyPair();
    /** WA noise protocol wrapper */
    let noise = (0, Utils_1.makeNoiseHandler)({
    ...
    let sendPromise = (0, util_1.promisify)(ws.send);
    /** send a raw buffer */
    ...
    const end = (error) => {
        ...
        // ⚡ AUTO-RECONNECT: Check if we should reconnect
        const statusCode = (error === null || error === void 0 ? void 0 : error.output) ? error.output.statusCode : undefined;
        const shouldReconnect = autoReconnect &&
            ...
            reconnectAttempts < maxReconnectRetries;
        ...
        if (shouldReconnect) {
            // ⚡ Don't remove listeners — we're reconnecting
            reconnectAttempts++;
            const delay = Math.min(baseReconnectDelayMs * Math.pow(1.5, reconnectAttempts - 1), 30000);
            logger.info({ attempt: reconnectAttempts, maxRetries: maxReconnectRetries, delayMs: delay, statusCode }, '⚡ auto-reconnecting...');
            reconnectTimer = setTimeout(reconnect, delay);
        } else {
            ev.removeAllListeners('connection.update');
        }
    };
    // ⚡ AUTO-RECONNECT: Create new WebSocket + noise + re-bind events
    const reconnect = () => {
        logger.info({ attempt: reconnectAttempts }, '⚡ reconnecting to WhatsApp...');
        // Reset internal state
        closed = false;
        epoch = 1;
        lastDateRecv = undefined;
        // New ephemeral key pair & noise handler
        ...
        ws = new Client_1.WebSocketClient(url, config);
        sendPromise = (0, util_1.promisify)(ws.send);
        // Re-bind all ws event handlers
        bindWsEvents();
        // Buffer events for reconnect
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "XZLYNEMD") => {
        if (pairKey) {
    ...
    // ⚡ All ws event binding extracted into function for auto-reconnect re-binding
    ...
    const bindWsEvents = () => {
        ...
        ws.on('CB:success', async (node) => {
            try {
                ...
                // Register LID bot sendiri ke mapping
                if (node.attrs.lid && authState.creds.me?.id) {
                    const { registerLidJid } = require('../WABinary/jid-utils');
                    registerLidJid(node.attrs.lid, authState.creds.me.id);
                }
    ...
    // Bind events for initial connection
    bindWsEvents();
    process.nextTick(() => {
        var _a;
