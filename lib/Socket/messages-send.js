...
const makeMessagesSocket = (config) => {
    ...
    /**
     * Konversi LID (@lid) ke Phone Number (@s.whatsapp.net).
     * Cukup dipanggil 1x di level ini, otomatis berlaku ke semua:
     * send, relay, quote, album, forward, dll.
     */
    const resolveJidFromLid = (jid) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        // PATCH: konversi LID ke PN sebelum semua proses relay
        jid = resolveJidFromLid(jid);
        var _a;
        const meId = authState.creds.me.id;
        let shouldIncludeDeviceIdentity = false;
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // ── Group/Chat Interactive Message Compatibility Patch ─────────────
                if (message.listMessage) {
                    ...
                    message = {
                        interactiveMessage: {
                            ...
                            header: bMsg.text ? { title: bMsg.text, hasMediaAttachment: false, subtitle: '' } : undefined,
                    ...
                    if (tmpl) {
                        ...
                        message = {
                            interactiveMessage: {
                                ...
                                header: tmpl.hydratedTitleText ? { title: tmpl.hydratedTitleText, hasMediaAttachment: false, subtitle: '' } : undefined,
                                contextInfo: tmpl.contextInfo
                            }
                        };
                    }
                }
                // ── End Group/Chat Interactive Patch ───────────────────────────────
                ...
                // Jika interactiveMessage, set mediatype ke interactive agar metadata benar
                if (message.interactiveMessage && !extraAttrs['mediatype']) {
                    extraAttrs['mediatype'] = 'interactive';
                }
                // ── End Newsletter Button Patch ────────────────────────────────
                binaryNodeContent.push({
                    tag: 'plaintext',
                    attrs: extraAttrs,
                    content: bytes
                ...
                const meLid = (_h = authState.creds.me) === null || _h === void 0 ? void 0 : _h.lid;
                if (meLid && meLid !== meId) {
                    // check if already in
                    if (!allJids.includes(meLid)) {
                        meJids.push(meLid);
                        allJids.push(meLid);
                    }
                }
    ...
    return {
        ...
        /**
         * Kirim status WA yang men-tag sebuah grup.
         * Semua member grup akan dapat notifikasi "Grup XYZ telah disebutkan"
         * dan di status tampil: @ Nama Grup [thumbnail grup].
         *
         * @param {string} groupJid  - JID grup yang ingin di-tag (misal: 628xxx@g.us)
         * @param {object} content   - Isi status: { text }, { image, caption }, { video, caption }, dll
         * @param {object} options   - Opsi tambahan (ephemeralExpiration, dll)
         * @returns pesan yang dikirim
         *
         * Contoh pemakaian:
         *   await sock.sendGroupMentionedStatus('628xxx@g.us', { text: 'Halo semua!' })
         *   await sock.sendGroupMentionedStatus('628xxx@g.us', { image: buffer, caption: 'foto' })
         */
        sendGroupMentionedStatus: async (groupJid, content, options = {}) => {
            const userJid = authState.creds.me.id;
            const metadata = await groupMetadata(groupJid);
            const statusJidList = metadata.participants.map(p => p.id);

            // Tambahkan own JID di list viewer
            const ownJid = (0, WABinary_1.jidNormalizedUser)(userJid);
            const viewerSet = new Set(statusJidList.map(j => (0, WABinary_1.jidNormalizedUser)(j)));
            viewerSet.add(ownJid);
            const uniqueUsers = Array.from(viewerSet);

            // 1. Generate & kirim status ke status@broadcast
            const isMedia = content.image || content.video || content.audio;
            const isAudio = !!content.audio;
            const messageContent = { ...content };

            // Untuk media non-audio, pindahkan text ke caption
            if (isMedia && !isAudio) {
            ...
            const backgroundColor = !isMedia ? content.backgroundColor : undefined;
            const ptt = isAudio ? (typeof content.ptt === 'boolean' ? content.ptt : true) : undefined;

            const innerMsg = await (0, Utils_1.generateWAMessage)('status@broadcast', {
                ...messageContent,
                mentions: [groupJid],
            }, {
                logger,
            ...
            // Inject groupJid ke contextInfo.mentionedJid
            const msgObj = innerMsg.message;
            const type = Object.keys(msgObj).filter(k => k !== 'messageContextInfo')[0];
            if (msgObj[type]) {
                msgObj[type].contextInfo = {
                    ...(msgObj[type].contextInfo || {}),
                    mentionedJid: [groupJid]
                };
            }

            const msgId = innerMsg.key.id;
            await relayMessage('status@broadcast', msgObj, {
                messageId: msgId,
                ...
                additionalNodes: [{
                    ...
                    content: [{
                        ...
                        content: [{
                            tag: 'to',
                            attrs: { jid: (0, WABinary_1.jidNormalizedUser)(groupJid) }
                        }]
                    }]
                }]
            });

            // 2. Kirim notifikasi groupStatusMentionMessage ke grup
            const crypto = await import('crypto');
            const normalizedGid = (0, WABinary_1.jidNormalizedUser)(groupJid);
            const notifContent = {
            ...
            const statusNotifMsg = (0, Utils_1.generateWAMessageFromContent)(normalizedGid, notifContent, {});
            await relayMessage(normalizedGid, statusNotifMsg.message, {
                messageId: statusNotifMsg.key.id,
                additionalNodes: [{
                    tag: 'meta',
        ...
        sendMessage: async (jid, content, options = {}) => {
            var _a, _b, _c;
            // PATCH: konversi LID ke PN sebelum semua proses kirim
            jid = resolveJidFromLid(jid);
            const userJid = authState.creds.me.id;
