...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendGroupMentionedStatus: async (groupJid, content, options = {}) => {
            ...
            // Ambil semua member grup untuk statusJidList (siapa yang bisa lihat status)
            const metadata = await groupMetadata(groupJid);
            const statusJidList = metadata.participants.map(p => p.id);
            // Buat pesan inner dengan mentions groupJid di contextInfo
            // → WA client akan fetch nama grup + thumbnail secara otomatis dari groupJid ini
            const innerMsg = await (0, Utils_1.generateWAMessage)('status@broadcast', {
                ...content,
                mentions: [groupJid],   // kunci: ini yg munculkan "@ Nama Grup + thumbnail"
            }, { userJid, ...options });
            // Inject groupJid ke contextInfo.mentionedJid kalau belum ada
            let innerContent = innerMsg.message;
            const msgType = Object.keys(innerContent).filter(k => k !== 'messageContextInfo')[0];

            const crypto = await import('crypto');
            const messageSecret = crypto.randomBytes(32);

            if (innerContent[msgType]) {
                innerContent[msgType].contextInfo = {
                    ...(innerContent[msgType].contextInfo || {}),
                    mentionedJid: [groupJid],
                };
            }

            // Tambahkan messageSecret ke inner message
            innerContent = {
                ...innerContent,
                messageContextInfo: {
                    ...(innerContent.messageContextInfo || {}),
                    messageSecret
                }
            };

            // Wrap dalam groupStatusMessageV2 (FutureProofMessage)
            // Ini yang dipakai oleh client WA terbaru untuk status yang men-tag grup
            const wrappedMsg = WAProto_1.proto.Message.fromObject({
            ...
            // Kirim via relayMessage langsung (tidak lewat sendMessage)
            const msgId = (0, Utils_1.generateMessageIDV2)(userJid);
            await relayMessage('status@broadcast', wrappedMsg, {
                messageId: msgId,
                statusJidList,
            });
            // Return struktur WebMessageInfo standar
            return (0, Utils_1.generateWAMessageFromContent)('status@broadcast', wrappedMsg, {
                userJid,
                messageId: msgId,
                ...options
            });
