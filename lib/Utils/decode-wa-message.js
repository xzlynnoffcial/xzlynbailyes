...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    // Jika chatId (remoteJid) masih @lid dan bukan grup, coba resolve pakai senderPn/remoteJidAlt
    if ((0, WABinary_1.isLidUser)(key.remoteJid) && !(0, WABinary_1.isJidGroup)(key.remoteJid)) {
        key.remoteJid = remoteJidAlt || senderPn || key.remoteJid;
    }
    // Jika participant masih @lid, coba resolve pakai participantAlt/senderPn
    if (key.participant && (0, WABinary_1.isLidUser)(key.participant)) {
        key.participant = participantAlt || senderPn || key.participant;
    }
    // Resolve author juga
    if ((0, WABinary_1.isLidUser)(author)) {
        author = participantAlt || senderPn || author;
    }
    return {
        fullMessage,
        author,
