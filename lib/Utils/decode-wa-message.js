...
const { registerLidJid, lidToJid } = require("../WABinary/jid-utils");
...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    // ── LID→PN: register mapping ke LID_MAP untuk future lookups ──
    if (senderPn && from && from.endsWith('@lid')) {
        registerLidJid(from, senderPn);
    }
    if (senderLid && senderPn) {
        registerLidJid(senderLid, senderPn);
    }
    if (participantLid && senderPn) {
        registerLidJid(participantLid, senderPn);
    }
    // ── LID→PN: konversi sedini mungkin ──
    if ((0, WABinary_1.isLidUser)(key.remoteJid) && !(0, WABinary_1.isJidGroup)(key.remoteJid)) {
        key.remoteJid = remoteJidAlt || senderPn || lidToJid(key.remoteJid) || key.remoteJid;
    }
    if (key.participant && (0, WABinary_1.isLidUser)(key.participant)) {
        key.participant = participantAlt || senderPn || lidToJid(key.participant) || key.participant;
    }
    if ((0, WABinary_1.isLidUser)(author)) {
        author = participantAlt || senderPn || lidToJid(author) || author;
    }
    return {
        fullMessage,
        author,
