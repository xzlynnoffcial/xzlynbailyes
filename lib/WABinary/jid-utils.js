...
const areJidsSameUser = (jid1, jid2) => {
    var _a, _b;
    const norm1 = (0, exports.lidToJid)(jid1) || jid1;
    const norm2 = (0, exports.lidToJid)(jid2) || jid2;
    return (((_a = (0, exports.jidDecode)(norm1)) === null || _a === void 0 ? void 0 : _a.user) === ((_b = (0, exports.jidDecode)(norm2)) === null || _b === void 0 ? void 0 : _b.user));
};
...
// LID → real JID mapping (populated by contacts events + creds.me)
const LID_MAP = new Map();
const registerLidJid = (lid, jid) => {
    if (!lid || !jid || lid === jid) return;
    const lidNum = lid.split(/[:@]/)[0];
    const jidNorm = jid.includes('@') ? jid.split(/[:@]/)[0] + '@s.whatsapp.net' : jid + '@s.whatsapp.net';
    LID_MAP.set(lidNum, jidNorm);
    LID_MAP.set(lid, jidNorm);
    if (lid.includes('@')) LID_MAP.set(lid.replace('@lid', '@s.whatsapp.net'), jidNorm);
};
exports.registerLidJid = registerLidJid;
exports.LID_MAP = LID_MAP;
const jidNormalizedUser = (jid) => {
    ...
    if (server === 'lid') {
        // Lookup real phone number dari map
        const mapped = LID_MAP.get(user) || LID_MAP.get(jid);
        if (mapped) return mapped;
        // Fallback: biarkan tetap sebagai LID
        return (0, exports.jidEncode)(user, 'lid');
    }
...
const lidToJid = (jid) => {
    ...
    const user = jid.split(/[:@_]/)[0];
    // Lookup real phone number dari map
    const mapped = LID_MAP.get(user) || LID_MAP.get(jid);
    if (mapped) return mapped;
    // Fallback: kembalikan jid asli (tetap LID)
    return jid;
};
